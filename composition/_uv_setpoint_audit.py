#!/usr/bin/env python3
"""
UV setpoint-refactor audit.

Assembles all 8 UV standard configs through the real server path
(assemble + inject_program_code) and checks the same failure classes the
plant audit checks, focused on the single-setpoint refactor:
...
Usage:
    ...
    PYTHONPATH=. venv/bin/python -m composition._uv_setpoint_audit
"""
...
def prog_refs(code):
    ...
    for line in code.splitlines():
        s = line.strip()
        # strip leading BASIC line number before checking for REM
...
def main():
    uv_ids = sorted(c for c in STANDARD_CONFIGS if c.startswith("SBS-UV-"))
    print(f"UV standard configs found: {len(uv_ids)} -> {uv_ids}\n")
    ...
    print()
    if total_fail:
        print(f"AUDIT RESULT: FAIL — {total_fail}/{len(uv_ids)} configs have issues")
        sys.exit(1)
    print(f"AUDIT RESULT: PASS — all {len(uv_ids)} UV configs assemble cleanly, "
