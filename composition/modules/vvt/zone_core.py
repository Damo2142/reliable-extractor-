...
{device-name}-NET-RTU-STATUS = {parent}DEV4194303:121
REM
REM --- Read Supply Fan Status ---
{device-name}-NET-RTU-SF-S = {parent}BI1
REM
REM --- Read Supply Air Temp ---
{device-name}-NET-SAT = {parent}AI2
REM
REM --- Read Global ESM Flag ---
{device-name}-NET-GLOBAL-ESM = {parent}BV23
REM
REM --- Read Occupancy Command ---
{device-name}-NET-SCHED = {parent}BV25
REM
REM --- Read HVAC Mode ---
{device-name}-NET-HVAC-MODE = {parent}MV13
REM
...
REM
{parent}AY1[ {device-name}-CFG-ZONE-ADDR ] = {device-name}-HVAC-MODE-VOTE
{parent}AY2[ {device-name}-CFG-ZONE-ADDR ] = {device-name}-ACT-RMT
{parent}AY3[ {device-name}-CFG-ZONE-ADDR ] = {device-name}-RMT-SP-DEV
{parent}AY4[ {device-name}-CFG-ZONE-ADDR ] = {device-name}-OCC-MODE
REM
"""
...
REM
REM --- Setpoint Limits ---
{device-name}-RMT-SP = LIMIT( {device-name}-RMT-SP, {device-name}-CFG-RMT-SP-LO-LIMIT, {device-name}-CFG-RMT-SP-HI-LIMIT )
REM
REM --- Heating and Cooling Setpoints ---
REM Deadband splits around operator setpoint
{device-name}-RMT-HTG-SP = {device-name}-RMT-SP - ( {device-name}-CFG-RMT-SP-DB / 2.0 )
{device-name}-RMT-CLG-SP = {device-name}-RMT-SP + ( {device-name}-CFG-RMT-SP-DB / 2.0 )
REM
REM --- Room Temp Deviation ---
{device-name}-RMT-SP-DEV = {device-name}-ACT-RMT - {device-name}-ACT-RMT-SP
REM
...
IF {device-name}-OCC-MODE <= 3 THEN {device-name}-ACT-RMT-SP = {device-name}-RMT-SP
IF {device-name}-OCC-MODE = 5 THEN {device-name}-ACT-RMT-SP = {device-name}-CFG-RMT-NSB-SP
IF {device-name}-OCC-MODE = 6 THEN {device-name}-ACT-RMT-SP = {device-name}-CFG-RMT-NSF-SP
IF {device-name}-OCC-MODE = 4 THEN {device-name}-ACT-RMT-SP = {device-name}-RMT-SP
REM
REM --- Damper Limits ---
{device-name}-CFG-DMP-MIN = LIMIT( {device-name}-CFG-DMP-MIN, 0.0, 100.0 )
{device-name}-CFG-DMP-MAX = LIMIT( {device-name}-CFG-DMP-MAX, 0.0, 100.0 )
{device-name}-CFG-DMP-FAIL = LIMIT( {device-name}-CFG-DMP-FAIL, 0.0, 100.0 )
{device-name}-CFG-DMP-ESM = LIMIT( {device-name}-CFG-DMP-ESM, 0.0, 100.0 )
REM
"""
...
IF {device-name}-CFG-SCHED-LOCAL THEN {device-name}-OCC-CMD = {device-name}-LOCAL-SCHED
IF NOT {device-name}-CFG-SCHED-LOCAL THEN {device-name}-OCC-CMD = {device-name}-NET-SCHED
REM
REM --- Base Mode from Schedule ---
IF {device-name}-OCC-CMD THEN {device-name}-OCC-MODE = 1
IF NOT {device-name}-OCC-CMD THEN {device-name}-OCC-MODE = 4
REM
...
IF {device-name}-OCC-MODE = 4 AND {device-name}-OCC-MOTION THEN {device-name}-OCC-BYPASS = 1
IF {device-name}-OCC-BYPASS THEN {device-name}-OCC-MODE = 2
IF {device-name}-OCC-BYPASS AND TIME-ON( {device-name}-OCC-BYPASS ) > {device-name}-CFG-BYPASS-SP THEN {device-name}-OCC-BYPASS = 0
IF {device-name}-OCC-MODE = 1 THEN {device-name}-OCC-BYPASS = 0
REM
REM --- Standby Mode ---
REM If occupied but no motion for extended period, enter standby
IF {device-name}-OCC-MODE = 1 AND NOT {device-name}-OCC-MOTION THEN {device-name}-OCC-STANDBY = TIME-ON( NOT {device-name}-OCC-MOTION ) > {device-name}-CFG-BYPASS-SP
IF {device-name}-OCC-STANDBY AND {device-name}-OCC-MODE = 1 THEN {device-name}-OCC-MODE = 3
IF {device-name}-OCC-MOTION THEN {device-name}-OCC-STANDBY = 0
REM
REM --- Night Setback ---
REM Room temp below NSB setpoint while unoccupied
IF {device-name}-OCC-MODE = 4 THEN NSB-ACT = SWITCH( {device-name}-ACT-RMT, {device-name}-CFG-RMT-NSB-SP, 1.0 )
IF {device-name}-OCC-MODE <> 4 THEN NSB-ACT = 0
IF NSB-ACT THEN {device-name}-OCC-MODE = 5
REM
REM --- Night Set-Forward ---
REM Room temp above NSF setpoint while unoccupied
IF {device-name}-OCC-MODE = 4 THEN NSF-ACT = SWITCH( {device-name}-ACT-RMT, {device-name}-CFG-RMT-NSF-SP, 1.0 )
IF {device-name}-OCC-MODE <> 4 THEN NSF-ACT = 0
IF NSF-ACT THEN {device-name}-OCC-MODE = 6
REM
"""
...
REM Last-write-wins precedence ladder for DMP-REQ, then OPEN/CLOSE/IDLE with deadband.
REM
REM --- Disable Firmware Flow Control (VVT uses room temp, not flow) ---
STOP BV6
BV6 = 0
REM
REM --- Detect local heating/cooling mode from SAT vs room temp ---
REM Cooling (HVAC-MODE-LOCAL=2) → loop direct: temp rises → damper opens.
REM Heating (HVAC-MODE-LOCAL=3/4) → loop reverse: temp drops → damper opens.
...
REM
REM --- Default: normal damper request from DMP-LOOP output ---
DMP-REQ = DMP-LOOP
DMP-REQ = LIMIT( DMP-REQ , CFG-DMP-MIN , CFG-DMP-MAX )
REM
REM --- Overrides (later writes take precedence — most-specific last) ---
...
REM
REM --- Drive MO7 to requested position with deadband ---
...
REM Cooling and heating branches are mutually exclusive (no overlap),
REM so last-write-wins ordering is safe.
REM
REM --- Default: ESM (neutral / energy savings) ---
HVAC-MODE-VOTE = CFG-VOTE-ESM-NORMAL
REM
REM --- Cooling votes (room above cooling SP) ---
IF ACT-RMT > RMT-CLG-SP THEN HVAC-MODE-VOTE = 10.0 + CFG-VOTE-CLG-NORMAL
IF ACT-RMT > RMT-CLG-SP AND ( ACT-RMT - RMT-CLG-SP ) > VOTE-NORMAL-DEV THEN HVAC-MODE-VOTE = 10.0 + CFG-VOTE-CLG-NORMAL + CFG-VOTE-CLG-DEMAND
IF ACT-RMT > RMT-CLG-SP AND ( ACT-RMT - RMT-CLG-SP ) > VOTE-DEMAND-DEV THEN HVAC-MODE-VOTE = 20.0
REM
REM --- Heating votes (room below heating SP) ---
IF ACT-RMT < RMT-HTG-SP THEN HVAC-MODE-VOTE = -1.0 * CFG-VOTE-HTG-NORMAL
IF ACT-RMT < RMT-HTG-SP AND ( RMT-HTG-SP - ACT-RMT ) > VOTE-NORMAL-DEV THEN HVAC-MODE-VOTE = -1.0 * ( CFG-VOTE-HTG-NORMAL + CFG-VOTE-HTG-DEMAND )
IF ACT-RMT < RMT-HTG-SP AND ( RMT-HTG-SP - ACT-RMT ) > VOTE-DEMAND-DEV THEN HVAC-MODE-VOTE = -10.0
REM
REM --- NSB / NSF Override (unoccupied modes 5 and 6) ---
...
REM Each alarm requires both: occupied (OCC-MODE<4) AND past 30-min settle.
REM
IF OCC-MODE < 4 AND TIME-ON( OCC-MODE < 4 ) >= 0:30:00 AND ACT-RMT > ( RMT-CLG-SP + 3.0 ) THEN ALARM = 1
IF OCC-MODE < 4 AND TIME-ON( OCC-MODE < 4 ) >= 0:30:00 AND ACT-RMT < ( RMT-HTG-SP - 3.0 ) THEN ALARM = 2
IF OCC-MODE < 4 AND TIME-ON( OCC-MODE < 4 ) >= 0:30:00 AND ABS( DMP-POS - DMP-REQ ) > ( CFG-DMP-DB * 1.5 ) THEN DALARM DMP-POS , 0:10:00
"""
