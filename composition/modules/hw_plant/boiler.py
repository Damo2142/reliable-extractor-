...
_BLR_HWST_BASE = 7       # AI: BLR1-HWST at IN7, BLR2 at IN8, etc. (if monitored)
...
def build_blr_cascade(num_boilers=2, analog_spt=True, monitor_hwst=False):
    ...
    # Per-boiler supply temp (optional)
    if monitor_hwst:
        for n in range(1, num_boilers + 1):
            inputs.append(InputPoint(
...
def build_blr_full(num_boilers=2, monitor_hwst=True):
    ...
    for n in range(1, num_boilers + 1):
        ...
        # Supply temp per boiler (full control always monitors)
        if monitor_hwst:
            inputs.append(InputPoint(
                _BLR_HWST_BASE + (n - 1) * 2, f"BLR{n}-HWST", "AI", "10K -40 ->250",
                f"Boiler {n} HW Supply Temperature", "°F"))
        ...
        values.append(ValuePoint(21 + (n-1)*3, f"BLR{n}-SS", "BV", False,
            f"Boiler {n} Start/Stop Status"))
        values.append(ValuePoint(22 + (n-1)*3, f"BLR{n}-FAIL", "BV", False,
            f"Boiler {n} Failure"))
        values.append(ValuePoint(23 + (n-1)*3, f"BLR{n}-ALARM-STS", "BV", False,
            f"Boiler {n} Alarm Status"))
