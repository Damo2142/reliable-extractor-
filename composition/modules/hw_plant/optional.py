...
def build_ahu_integration(num_ahus=2):
    ...
    d = "{device-name}"
    ahu_pos_list = " , ".join(f"{d}-AHU{n}-HWV-POS" for n in range(1, num_ahus + 1))
    ahu_htg_list = " + ".join(f"{d}-AHU{n}-HTG-REQS" for n in range(1, num_ahus + 1))
    net_data_code = "REM **** Network Data Reads from AHUs\n"
    net_data_code += "REM **** Generated by SBS Composition Engine v2 — HW Plant\n"
    net_data_code += f"REM **** Dynamic for {num_ahus} AHU(s)\n\n"
    net_data_code += f"{d}-NET-HWV-POS-MIN = MIN( {ahu_pos_list} )\n"
    net_data_code += f"{d}-NET-HWV-POS-MAX = MAX( {ahu_pos_list} )\n"
    net_data_code += f"{d}-NET-HWV-POS-AVG = AVG( {ahu_pos_list} )\n"
    net_data_code += f"{d}-TOTAL-HTG-REQS = {ahu_htg_list}\n"

    programs = [
        ProgramDef(31, "HW-NET-DATA-PRG", "HW-PRG31-NET-DATA.bas",
            net_data_code, True,
