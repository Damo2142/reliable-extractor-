...
def build_chiller(num_chillers=2):
    ...
    # Generate CHLR-CTRL code dynamically based on num_chillers
    d = "{device-name}"
    ctrl_code = f"REM **** Chiller Control — {num_chillers} chiller(s)\n"
    ctrl_code += "REM **** Generated by SBS Composition Engine v2 — CHW Plant\n\n"
    for n in range(1, num_chillers + 1):
        ctrl_code += f"REM **** === Chiller {n} === ****\n"
        ctrl_code += f"IF {d}-CHLR{n}-OOS THEN STOP {d}-CHLR{n}-S/S , END\n"
        ctrl_code += f"IF {d}-CHLR{n}-ALARM THEN STOP {d}-CHLR{n}-S/S , START {d}-CHLR{n}-FAIL , END\n"
        ctrl_code += f"IF NOT {d}-CHLR{n}-S/S AND TIME-OFF( {d}-CHLR{n}-S/S ) < {d}-CHLR-MIN-OFF-TIME THEN END\n"
        if num_chillers == 1:
            ctrl_code += f"IF {d}-CHW-SYS-ENAB AND {d}-ANY-CHWP-ON AND NOT {d}-CHLR{n}-ALARM AND NOT {d}-CHLR{n}-OOS THEN START {d}-CHLR{n}-S/S ELSE STOP {d}-CHLR{n}-S/S\n"
        else:
            ctrl_code += f"IF NOT ({d}-CHW-SYS-ENAB AND {d}-ANY-CHWP-ON) THEN STOP {d}-CHLR{n}-S/S , END\n"
            ctrl_code += f"IF {d}-LEAD-CHLR = {n} THEN A = {d}-LEAD-CHLR-SS\n"
            ctrl_code += f"IF {d}-LEAD-CHLR <> {n} THEN A = {d}-LAG-CHLR-SS\n"
            ctrl_code += f"IF A AND NOT {d}-CHLR{n}-OOS AND NOT {d}-CHLR{n}-ALARM THEN START {d}-CHLR{n}-S/S ELSE STOP {d}-CHLR{n}-S/S\n"
        ctrl_code += f"B = SWITCH( B , TIME-ON( {d}-CHLR{n}-S/S ) , 0 , {d}-CHLR-ALARM-DELAY )\n"
        ctrl_code += f"IF B AND {d}-CHLR{n}-ALARM THEN START {d}-CHLR{n}-FAIL\n"
        ctrl_code += f"IF NOT {d}-CHLR{n}-ALARM THEN STOP {d}-CHLR{n}-FAIL\n"
        ctrl_code += f"{d}-CHLR{n}-CHWS-SP = {d}-ACT-CHWS-TEMP-SP\n"
        ctrl_code += f"IF {d}-RESET-SAFETIES THEN STOP {d}-CHLR{n}-FAIL\n\n"

    programs.append(ProgramDef(6, "CHW-CHLR-CTRL-PRG", "CHW-PRG06-CHLR-CTRL.bas",
        ctrl_code, True,
