...
def build_ahu_integration(num_ahus=2):
    ...
    for n in range(1, num_ahus + 1):
        ...
        ahu_cfg += "REM **** Generated by SBS Composition Engine v2 — CHW Plant\n"
        ahu_cfg += f"REM **** Reads zone data from AHU {n}, writes CHW availability\n\n"
        ...
        ahu_cfg += f"{d}-AHU{n}-CLG-REQS = {p}-CLG-REQS\n"
        ahu_cfg += f"{d}-AHU{n}-CLG-DMD = {p}-CLG-DMD\n"
        ahu_cfg += f"{d}-AHU{n}-CHV-POS = {p}-CHV-POS\n\n"
        ahu_cfg += f"REM **** Write CHW availability to AHU ****\n"
        ahu_cfg += f"IF {d}-CHW-AVAIL THEN START {d}-AHU{n}-NET-OCC-CMD\n"
        ahu_cfg += f"IF NOT {d}-CHW-AVAIL THEN STOP {d}-AHU{n}-NET-OCC-CMD\n"
        programs.append(ProgramDef(45 + n, f"CHW-AHU{n}-CFG-PRG",
            f"CHW-PRG{45 + n}-AHU{n}-CFG.bas", ahu_cfg, True,
