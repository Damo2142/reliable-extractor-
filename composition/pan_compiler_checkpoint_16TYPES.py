...
def write_sys_group_block(instance: int, name: str) -> bytes:
    """Build a SYS_GROUP (type 141) block."""
    records = bytearray(b'\x00\x00\x00')
    records += _rec_uint8(0x0000, 0x1C, 0x71, 0x00)              # description (null)
    records += _rec_mu_header(name)                                # Mu name
    records += _rec_uint8(0x0000, 0x75, 0x91, 0x3A)              # units = 0x3A (58)

    # Vendor props — grid data placeholder (empty for now)
    # 0x0428 is complex grid layout — skip for initial compiler
